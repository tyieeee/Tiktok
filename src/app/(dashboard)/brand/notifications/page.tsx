  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Stay updated on applications and collaborations.</p>
      </div>
        ...
        {notifs.data?.map((n: any) => (
          <Card key={n.id} className={cn("transition-colors", !n.readAt && "border-primary/20 bg-primary/5")}>
            <CardContent className="flex items-start gap-4 p-4">
              ...
              {!n.readAt && <div className="h-2 w-2 shrink-0 rounded-full bg-primary" />}
            </CardContent>
          </Card>
        ))}
      </div>
